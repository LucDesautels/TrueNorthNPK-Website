...
def main():
    ...
    # Gallery photos supplied directly as WebP (already optimized upstream, with
    # no JPG/PNG source to process). We don't re-encode them, but we still record
    # their dimensions + an LQIP so js/img.js reserves space and shows the same
    # blur-up as everything else. Files that have a JPG/PNG source are skipped -
